...
| 11 | `s11-footer` | [s11-footer.html](s11-footer.html) | `MERGED` | 2026-06-10 | Gmail + orders@ + Instagram icons in contact block · no bottom bar social · v2.21 |
...
| v2.25 | 2026-06-11 | CMS system — `content.json` (all editable content) · `admin/index.html` + `admin/config.yml` (Decap CMS) · `netlify.toml` · `index.html` content loader: menu rebuilt from JSON · stats countUp reads from JSON · modal popular items from JSON · hours + phone + email dynamic · Featured cards dynamic · All animations intact |
